...
# # home system
# wd = "/Users/kericlamb/Documents/Work MacBook/Research/paper_code/H_annuus_demography/H_annuus_final/code/moments_scripts/" # upper level directory
...
if template == True:
    ...
    self_TV3_rate = "{self_TV3_rate}"
    Nmodcon_TV = "{Nmodcon_TV}" # exp_con only
    Tmodcon_TV = "{Tmodcon_TV}" # exp_con only
    ...
    self_TV3_rate = self_TV0_rate
    Nmodcon_TV = "4e4" # exp_con only
    Tmodcon_TV = "361" # exp_con only
...
if template == True:
    file_name = str(model + "_" + complexity + "_" + ancient_exp + "_" + shelf + "_" + TV_exp + "_" + "TV_" + selfing + "_WildAnc_" + selfing_all + "_template.yaml")
elif template == False:
    file_name = str(model + "_" + complexity + "_" + ancient_exp + "_" + shelf + "_" + TV_exp + "_" + "TV_" + selfing + "_WildAnc_" + selfing_all + "_model.yaml")
...
generation_time: 1
demes:
""".format(model + " + " + complexity + " + " + ancient_exp + " + " + shelf + " + " + TV_exp + " + TV " + selfing + " + WildAnc " + selfing_all)
...
if TV_exp == "TV_noexp":
    ...
    - end_time: {2}
      end_size: {1}{0}
    - end_time: 0
      end_size: {3}{4}""".format(f"\n      selfing_rate: {self_TV1_rate if shelf == 'noshelf' else self_TV2_rate}" if selfing == "self" else "", 
                                 Nexp_TV, Tmodcon_TV, Nmodcon_TV,
                                 f"\n      selfing_rate: {self_TV2_rate if shelf == 'noshelf' else self_TV3_rate}" if selfing == "self" else "")
elif TV_exp == "TV_bottle":
    # print("this could should not be running")
    if shelf == "noshelf":
        TV_shelf = """
    - end_time: {3}
    ...
    - end_time: 0
      end_size: {4}{5}""".format("\n      selfing_rate: {0}".format(self_TV0_rate) if selfing == "self" else "", 
                                 N0_TV, Ncon_TV, Texp_TV, Ncon2_TV,
                                 "\n      selfing_rate: {0}".format(self_TV1_rate) if selfing == "self" else "")
    elif shelf == "shelf":
        # print("this could should not be running")
        TV_shelf = """
    - end_time: {2}
    ...
    - end_time: 0
      end_size: {5}{1}""".format("\n      selfing_rate: {0}".format(self_TV0_rate) if selfing == "self" else "", 
                                 "\n      selfing_rate: {0}".format(self_TV2_rate) if selfing == "self" else "", 
                                 Tshelf_TV, N0_TV, Nshelf_TV, Ncon_TV, Texp_TV, Ncon_TV,
                                 "\n      selfing_rate: {0}".format(self_TV1_rate) if selfing == "self" else "")
    TVexp = """"""
