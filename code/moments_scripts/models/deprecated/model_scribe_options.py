...
sys.path.append(wd)
from options_bounds_config import *
...
- name: {self0_rate}
  ...
  - demes:
      TV:
        epochs:
          0: selfing_rate
  upper_bound: {self0_rate_upper}
  lower_bound: {self0_rate_lower}""".format(self0_rate=self0_rate, self0_rate_upper=self0_rate_upper, self0_rate_lower=self0_rate_lower)
Tshelf_TV_param = """
- name: {Tshelf_TV}
...
- name: {self1_rate}
  ...
  - demes:
      TV:
        epochs:
          1: selfing_rate
  upper_bound: {self1_rate_upper}
  lower_bound: {self1_rate_lower}""".format(self1_rate=self1_rate, self1_rate_upper=self1_rate_upper, self1_rate_lower=self1_rate_lower)
Tcon_exp_TV_param = """
- name: {Tcon_exp_TV}
...
- name: {self2_rate}
  ...
  - demes:
      TV:
        epochs:
          2: selfing_rate
  upper_bound: {self2_rate_upper}
  lower_bound: {self2_rate_lower}""".format(self2_rate=self2_rate, self2_rate_upper=self2_rate_upper, self2_rate_lower=self2_rate_lower)
# Migration
Mig_WildTV_param = """
- name: {Mig_WildTV}
...
anc_epoch0 = T0_anc_param + N0_anc_param
if ancient_exp == "anc_exp":
    anc_epoch1 = T1_anc_param + N1_anc_param
    anc_constraints = T1_anc_constraint + Tfinal_anc_constraint
elif ancient_exp == "anc_noexp":
    anc_epoch1 = ""
    anc_constraints = Tfinal_anc_constraint
anc_epoch2 = Tfinal_anc_param + Nfinal_anc_param
anc_params = anc_epoch0 + anc_epoch1 + anc_epoch2

# wild population options
if complexity == "base":
    Wild_params = N_Wild_param
    Wild_constraints = ""
elif complexity == "hyper":
    Wild_params = T0_Wild_param + N0_Wild_param + N0exp_Wild_param + Tcon_Wild_param + Ncon_Wild_param + Nfinal_Wild_param
    Wild_constraints = T0_Wild_constraint + Tcon_Wild_constraint

# TV population options
if TV_exp == "TV_noexp":
    if shelf == "noshelf":
        TV_params = "{0}{1}{2}".format(N0_TV_param, Ncon_TV_param.format("0"), self0_rate_param if selfing == "self" else "") # epoch 0
        TV_constraints = ""
    elif shelf == "shelf":
        TV_params = "{0}{1}{2}{3}".format(Tshelf_TV_param, N0_TV_param, Nshelf_TV_param, self0_rate_param if selfing == "self" else "") # epoch 0
        TV_params = "{0}{1}{2}".format(TV_params, Ncon_TV_param.format("1"), self1_rate_param if selfing == "self" else "")  # epoch 1
        TV_constraints = Tshelf_TV_constraint
elif TV_exp == "TV_exp":
    if shelf == "noshelf":
        TV_params = "{0}{1}{2}{3}".format(Tcon_exp_TV_param, N0_TV_param, Ncon_TV_param.format("0"), self0_rate_param if selfing == "self" else "") # epoch 0
        TV_params = "{0}{1}{2}".format(TV_params, Nexp_TV_param.format("1"), self1_rate_param if selfing == "self" else "") # epoch 1
        TV_constraints = Tcon_exp_TV_constraint
    elif shelf == "shelf":
        TV_params = "{0}{1}{2}{3}".format(Tshelf_TV_param, N0_TV_param, Nshelf_TV_param, self0_rate_param if selfing == "self" else "") # epoch 0
        TV_params = "{0}{1}{2}{3}".format(TV_params, Texp_TV_param, Ncon_TV_param.format("1"), self1_rate_param if selfing == "self" else "") # epoch 1
        TV_params = "{0}{1}{2}".format(TV_params, Nexp_TV_param.format("2"), self2_rate_param if selfing == "self" else "") # epoch 2
...
file_name = str(model + "_" + complexity + "_" + ancient_exp + "_" + shelf + "_" + TV_exp + "_" + selfing + "_options.yaml")
